...
export default function EditCoursePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [courseRes, teachersData] = await Promise.all([
        ...
        if (courseRes) {
          ...
          if (courseRes.teacher && !allTeachers.some((t: any) => t.id === courseRes.teacherId)) {
            allTeachers = [
              { 
                id: courseRes.teacherId, 
                name: `${courseRes.teacher.name} (${courseRes.teacher.role})`, 
                email: courseRes.teacher.email 
              }, 
              ...allTeachers
            ];
          }
          ...
          setFormData({
            ...
            price: courseRes.price?.toString() || "0",
            teacherId: courseRes.teacherId || "",
            thumbnail: courseRes.thumbnail || "",
            status: courseRes.status || "draft",
          });
        }
      } catch (error) {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Edit Kursus</h1>
          <p className="text-muted-foreground">Ubah informasi kursus</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card className="border-0 shadow-md">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input value={formData.shortDescription} onChange={(e) => setFormData({...formData, shortDescription: e.target.value})} placeholder="Deskripsi singkat dalam 1 kalimat" />
            </div>
            <div className="space-y-2">
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button type="submit" disabled={isSubmitting || uploadingImage} className="gradient-primary text-white shadow-lg shadow-primary/25 gap-2">
            {isSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Simpan Perubahan
          </Button>
        </div>
      </form>
    </div>
